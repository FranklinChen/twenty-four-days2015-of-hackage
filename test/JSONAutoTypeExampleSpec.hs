...
-- | Copied from WreqExampleSpec.
module JSONAutoTypeExampleSpec where

import JSONAutoTypeExample (getMeetupEventInfos)
import WreqExample (GroupId, eventName, venueName)
...
spec =
  describe "json-autotype" $ do
    it "there are named, located Pittsburgh Code and Supply events coming up" $ do
      ...
      Right events <- getMeetupEventInfos pittsburghCodeAndSupplyId
